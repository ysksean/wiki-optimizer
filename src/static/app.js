...
const I18N = {
  ko: {
    ...
    "arm_evolve": "진화", "arm_evolve-informed": "진화 · 근거 제공", "arm_evolve-incremental": "진화 · 증분 조직", "arm_control": "대조군 · 무진화", "arm_evolve-nohist": "진화 · 이력 없음", "arm_evolve-wiki": "진화 · 패턴 위키",
    ...
    bl_title: "정답률 비교 기준", bl_note: n => `같은 질문 ${n}개를 문서 없이, 그리고 원본을 읽고 답해 본 정답률`, bl_floor: "문서 없이", bl_ours: "이 구조", bl_wiki: "지금 위키", bl_ceiling: "원본 전체", bl_ceiling_doc: "원본 문서", bl_headroom: d => `원본 전체보다 ${d} 낮아요. 구조가 원본 내용을 잃었거나 답이 있는 파일을 못 찾은 몫이에요.`, bl_near_ceiling: d => `원본 전체와의 차이가 ${d}예요. 이 질문들로는 더 올라갈 자리가 거의 없어요.`, bl_headroom_wiki: d => `원본 문서로 답할 때보다 ${d} 낮아요. 위키에 원본 내용이 빠졌거나 라우터가 답이 있는 페이지를 못 찾은 몫이에요.`, bl_near_wiki: d => `원본 문서로 답할 때와의 차이가 ${d}예요. 이 질문들로는 위키를 더 다듬어도 올라갈 자리가 거의 없어요.`, bl_all_closed: "이 질문들은 모두 문서 없이도 풀려서, 이 점수로는 구조나 위키의 차이를 가를 수 없어요. 원본이 일반 지식에 가까우면 이렇게 돼요.", bl_below_floor: "문서 없이 답한 것보다 낮아요. 채점은 읽은 내용만 근거로 답하게 해서, 답이 있는 페이지를 못 찾으면 모델이 아는 내용도 '모름'이 돼요.", bl_misses: n => `원본을 다 읽고도 못 푼 질문 ${n}개 — 질문이나 정답이 잘못됐을 수 있어요`, bl_dropped: n => `문서 없이 풀리는 질문 ${n}개는 평가에서 뺐어요`, bl_kept: n => `문서 없이 풀리는 질문 ${n}개는 질문이 모자라 그대로 썼어요`, th_ceiling: "원본으로", selection_hero: (k, n) => `최종 후보 ${k}개를 ${n}회씩 다시 잰 값`, selection_title: "최종 후보 다시 재기", selection_note: n => `세대별 점수는 한 번 채점한 값이라 운이 섞여 있어요. 점수가 높은 후보만 인용형 답변으로 ${n}회씩 다시 채점해 골랐어요. 단일 채점 → 다시 잰 값:`, structure_hero: n => `${n}차 시도 종합`, attempt_n: n => `${n}차 시도`, attempt_meta: (f, a) => `파일 ${f}개 · 정확도 ${a}`,
  ...
  en: {
    ...
    "arm_evolve": "evolve", "arm_evolve-informed": "evolve · with evidence", "arm_evolve-incremental": "evolve · incremental", "arm_control": "control · no evolution", "arm_evolve-nohist": "evolve · no history", "arm_evolve-wiki": "evolve · pattern wiki",
    ...
    bl_title: "Accuracy reference points", bl_note: n => `the same ${n} questions answered with no documents and from the source`, bl_floor: "No documents", bl_ours: "This structure", bl_wiki: "Current wiki", bl_ceiling: "Full source", bl_ceiling_doc: "Source document", bl_headroom: d => `${d} below the full source: content the structure lost, or files the router did not find.`, bl_near_ceiling: d => `Within ${d} of the full source. These questions leave little room to improve.`, bl_headroom_wiki: d => `${d} below answering from the source documents: content missing from the wiki, or pages the router did not find.`, bl_near_wiki: d => `Within ${d} of answering from the source documents. These questions leave little room to improve the wiki.`, bl_all_closed: "Every one of these questions can be answered without the documents, so this score cannot tell structures or wikis apart. This happens when the sources are close to general knowledge.", bl_below_floor: "Lower than answering with no documents: scoring answers only from what was read, so when the right page is not found even facts the model knows become \"unknown\".", bl_misses: n => `${n} question(s) not answered even from the full source; the question or answer may be wrong`, bl_dropped: n => `${n} question(s) answerable without the documents were excluded`, bl_kept: n => `${n} question(s) answerable without the documents were kept because there were too few questions`, th_ceiling: "From source", selection_hero: (k, n) => `re-scored: top ${k} candidates × ${n} rounds`, selection_title: "Final candidates, re-scored", selection_note: n => `Per-attempt scores come from a single noisy scoring. The top candidates were re-scored ${n} times with quoted answers before choosing. Single score → re-scored:`, structure_hero: n => `attempt ${n} total`, attempt_n: n => `Attempt ${n}`, attempt_meta: (f, a) => `${f} files · accuracy ${a}`,
  ...
  zh: {
    ...
    "arm_evolve": "进化", "arm_evolve-informed": "进化 · 提供依据", "arm_evolve-incremental": "进化 · 增量组织", "arm_control": "对照组 · 不进化", "arm_evolve-nohist": "进化 · 无历史", "arm_evolve-wiki": "进化 · 模式 wiki",
    ...
    bl_title: "准确率参照", bl_note: n => `同样 ${n} 个问题在无文档、读原文时的准确率`, bl_floor: "无文档", bl_ours: "此结构", bl_wiki: "当前 wiki", bl_ceiling: "完整原文", bl_ceiling_doc: "原文文档", bl_headroom: d => `比完整原文低 ${d}，这是结构丢失的内容或路由没找到的文件造成的。`, bl_near_ceiling: d => `与完整原文相差 ${d}。这些问题几乎没有提升空间。`, bl_headroom_wiki: d => `比用原文文档作答低 ${d}，这是 wiki 缺少原文内容或路由没找到正确页面造成的。`, bl_near_wiki: d => `与用原文文档作答相差 ${d}。这些问题下 wiki 几乎没有提升空间。`, bl_all_closed: "这些问题不看文档也都能答对，因此这个分数无法区分结构或 wiki 的好坏。原文接近常识时会出现这种情况。", bl_below_floor: "低于无文档作答：评分只允许依据读到的内容作答，找不到正确页面时，即使模型知道的内容也会答“不知道”。", bl_misses: n => `读完整原文也答不出的问题 ${n} 个——问题或答案可能有误`, bl_dropped: n => `无需文档即可回答的 ${n} 个问题已排除`, bl_kept: n => `因问题不足，保留了 ${n} 个无需文档即可回答的问题`, th_ceiling: "用原文", selection_hero: (k, n) => `前 ${k} 个候选各重新评分 ${n} 次`, selection_title: "最终候选重新评分", selection_note: n => `各代分数只评了一次，含有运气成分。仅对高分候选用引用式回答各重新评分 ${n} 次后选出。单次评分 → 重新评分:`, structure_hero: n => `第 ${n} 次尝试综合`, attempt_n: n => `第 ${n} 次尝试`, attempt_meta: (f, a) => `${f} 个文件 · 准确率 ${a}`,
...
function routerAuditView(res, status) {
  ...
  if (res.accuracy != null)
    html += baselineBlock(res.baselines, null, res.accuracy, "bl_wiki", "bl_ceiling_doc");
  ...
  if (res.questions?.length) {
    const hasCeiling = res.questions.some(d => d.ceiling != null);
    html += `<details><summary>${t("r_questions")}</summary>${wrapTable(
      `<tr><th>${t("th_src_doc")}</th><th>${t("th_q")}</th><th>${t("th_picked")}</th><th>${t("th_chars")}</th><th>${t("th_correct")}</th>${hasCeiling ? `<th>${t("th_ceiling")}</th>` : ""}</tr>` +
      res.questions.map(d => `<tr><td>${esc(d.doc)}</td><td>${esc(d.q)}</td>
        <td>${(d.picked||[]).map(esc).join(", ")}</td><td>${d.read_chars}</td>
        <td>${d.score != null ? (d.score ? "⭕" : "❌") : "-"}</td>${hasCeiling ? `<td>${d.ceiling != null ? (d.ceiling ? "⭕" : "❌") : "-"}</td>` : ""}</tr>`).join(""))}</details>`;
  }
  return html + "</div>";
}
...
function structureRun(run, jobId) {
  ...
  html += baselineBlock(rep?.baselines || p.baselines, rep?.question_filter || p.question_filter,
    winner ? winner.accuracy : bestH?.score?.accuracy, "bl_ours", "bl_ceiling");
  if (cur) {
...
// 정답률 비교 기준 — 같은 질문을 문서 없이(아래) / 원본 전체로(위) 답한 정답률 사이에 이 결과를 놓는다.
// base: question_filter.baselines 또는 진단의 baselines, filter: 질문을 거른 기록
function baselineBlock(base, filter, acc, oursKey, ceilingKey) {
  const floor = base?.floor, ceil = base?.ceiling;
  const dropped = filter?.dropped ?? base?.dropped ?? 0;
  if (!floor && !ceil && !dropped) return "";
  const pos = v => `${(Math.max(0, Math.min(1, v)) * 100).toFixed(1)}%`;
  const marks = [
    floor && { cls: "floor", v: floor.accuracy, label: t("bl_floor") },
    acc != null && { cls: "ours", v: acc, label: t(oursKey) },
    ceil && { cls: "ceiling", v: ceil.accuracy, label: t(ceilingKey) },
  ].filter(Boolean);
  const n = ceil?.n ?? floor?.n ?? base?.n;
  let html = `<div class="baseline"><div class="bl-head"><b>${t("bl_title")}</b>${n ? `<span>${t("bl_note", n)}</span>` : ""}</div>`;
  if (marks.length > 1 || (marks.length && marks[0].cls !== "ours"))
    html += `<div class="bl-track" aria-hidden="true">${marks.map(m => `<i class="bl-mark ${m.cls}" style="left:${pos(m.v)}"></i>`).join("")}</div>
      <ul class="bl-legend">${marks.map(m => `<li class="${m.cls}"><i aria-hidden="true"></i>${m.label} <b>${m.v}</b></li>`).join("")}</ul>`;
  if (ceil && acc != null) {
    const gap = Math.round((ceil.accuracy - acc) * 1000) / 1000;
    const wiki = oursKey === "bl_wiki";
    html += `<p>${t(gap <= 0.05 ? (wiki ? "bl_near_wiki" : "bl_near_ceiling") : (wiki ? "bl_headroom_wiki" : "bl_headroom"), gap.toFixed(3))}</p>`;
  }
  if (floor && floor.accuracy >= 1) html += `<p class="warn">${t("bl_all_closed")}</p>`;
  else if (floor && acc != null && acc < floor.accuracy) html += `<p>${t("bl_below_floor")}</p>`;
  const qText = q => esc(typeof q === "string" ? q : `${q.doc} · ${q.q}`);
  if (ceil?.misses?.length)
    html += `<details><summary>${t("bl_misses", ceil.misses.length)}</summary><ul>${ceil.misses.map(q => `<li>${qText(q)}</li>`).join("")}</ul></details>`;
  if (dropped)
    html += filter?.dropped_questions?.length
      ? `<details><summary>${t("bl_dropped", dropped)}</summary><ul>${filter.dropped_questions.map(q => `<li>${qText(q)}</li>`).join("")}</ul></details>`
      : `<p class="muted">${t("bl_dropped", dropped)}</p>`;
  if (filter?.kept_closed_book) html += `<p class="muted">${t("bl_kept", filter.kept_closed_book)}</p>`;
  return html + "</div>";
}
