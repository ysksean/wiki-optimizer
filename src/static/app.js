...
const I18N = {
  ko: {
    ...
    bl_title: "정답률 비교 기준", bl_note: n => `같은 질문 ${n}개를 문서 없이, 원본 전체로 답해 본 정답률`, bl_floor: "문서 없이", bl_ours: "이 구조", bl_wiki: "지금 위키", bl_ceiling: "원본 전체", bl_ceiling_doc: "원본 문서", bl_headroom: d => `원본 전체보다 ${d} 낮아요. 구조가 원본 내용을 잃었거나 답이 있는 파일을 못 찾은 몫이에요.`, bl_near_ceiling: d => `원본 전체와의 차이가 ${d}예요. 이 질문들로는 더 올라갈 자리가 거의 없어요.`, bl_misses: n => `원본을 다 읽고도 못 푼 질문 ${n}개 — 질문이나 정답이 잘못됐을 수 있어요`, bl_dropped: n => `문서 없이 풀리는 질문 ${n}개는 평가에서 뺐어요`, bl_kept: n => `문서 없이 풀리는 질문 ${n}개는 질문이 모자라 그대로 썼어요`, th_ceiling: "원본으로", selection_hero: (k, n) => `최종 후보 ${k}개를 ${n}회씩 다시 잰 값`, selection_title: "최종 후보 다시 재기", selection_note: n => `세대별 점수는 한 번 채점한 값이라 운이 섞여 있어요. 점수가 높은 후보만 인용형 답변으로 ${n}회씩 다시 채점해 골랐어요. 단일 채점 → 다시 잰 값:`, structure_hero: n => `${n}차 시도 종합`, attempt_n: n => `${n}차 시도`, attempt_meta: (f, a) => `파일 ${f}개 · 정확도 ${a}`,
  ...
  zh: {
    ...
    bl_title: "准确率参照", bl_note: n => `同样 ${n} 个问题在无文档、读完整原文时的准确率`, bl_floor: "无文档", bl_ours: "此结构", bl_wiki: "当前 wiki", bl_ceiling: "完整原文", bl_ceiling_doc: "原文文档", bl_headroom: d => `比完整原文低 ${d}，这是结构丢失的内容或路由没找到的文件造成的。`, bl_near_ceiling: d => `与完整原文相差 ${d}。这些问题几乎没有提升空间。`, bl_misses: n => `读完整原文也答不出的问题 ${n} 个——问题或答案可能有误`, bl_dropped: n => `无需文档即可回答的 ${n} 个问题已排除`, bl_kept: n => `因问题不足，保留了 ${n} 个无需文档即可回答的问题`, th_ceiling: "用原文", selection_hero: (k, n) => `前 ${k} 个候选各重新评分 ${n} 次`, selection_title: "最终候选重新评分", selection_note: n => `各代分数只评了一次，含有运气成分。仅对高分候选用引用式回答各重新评分 ${n} 次后选出。单次评分 → 重新评分:`, structure_hero: n => `第 ${n} 次尝试综合`, attempt_n: n => `第 ${n} 次尝试`, attempt_meta: (f, a) => `${f} 个文件 · 准确率 ${a}`,
...
function baselineBlock(base, filter, acc, oursKey, ceilingKey) {
  ...
  if (ceil && acc != null) {
    ...
    html += `<p>${t(gap <= 0.05 ? "bl_near_ceiling" : "bl_headroom", gap.toFixed(3))}</p>`;
  }
