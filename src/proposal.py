...
def propose(task, entries, strategy, gap_questions=()):
    ...
    prompt = (
        ...
        "기본 골격은 Karpathy LLM Wiki 방식이다: 원본은 raw/에 보존하고, "
        "파생 지식은 wiki/에 두며 wiki/index.md로 탐색한다. 이 요청에서는 "
        "wiki/ 내부의 지식 페이지 구성을 설계한다. raw/와 운영 로그는 지식 페이지로 "
        "생성하지 말고, 탐색용 index.md는 본문 생성 단계에서 별도로 만든다. "
        "태스크에 다른 레이아웃이 명시되어 있으면 그 요구를 우선한다.\n"
        f"[분할 전략]\n{strategy}\n\n"
        "규칙:\n"
        "- 기본 path는 wiki/ 내부 상대경로 .md (예: operations/recovery.md; "
        "wiki/ 접두사는 붙이지 않는다). 슬래시로 하위 폴더를 표현한다.\n"
        "- 파일 수와 폴더 깊이는 태스크에 필요한 만큼 정한다. 단일 계층도 허용하며, "
        "질문에 답하는 데 이유가 없는 분할이나 고정 개수 맞추기는 하지 마라.\n"
